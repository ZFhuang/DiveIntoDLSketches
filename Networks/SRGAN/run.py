...
from SRGAN import SRGAN_generator,SRGAN_discriminator,Adversarial_loss
...
SAVE_PATH = rootPath+r'/Models/SRGAN_'+computer_name+r'_'+format_time+r'.pth'
LOAD_PATH = rootPath+r'/Models/SRGAN_huangtengcong_2021-04-01_20-50-06.pth'
...
# 初始化两个网络
generator = SRGAN_generator(scale=ratio,in_channel=1, num_filter=64, num_resiblk=16)
generator.apply(weights_init_kaiming)
logging.debug('\n'+str(generator))
logging.info('Generator inited!')
discriminator = SRGAN_discriminator(in_channel=1)
discriminator.apply(weights_init_kaiming)
logging.debug('\n'+str(discriminator))
logging.info('Discriminator inited!')

# 训练参数
lr, num_epochs = 0.0003, 1000
batch_size = 64
optim_gene = torch.optim.Adam(generator.parameters(), lr=lr)
optim_disc = torch.optim.Adam(discriminator.parameters(), lr=lr)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    optim_gene, mode='min', factor=0.1, patience=30, verbose=True)
loss_gene = Adversarial_loss()
loss_disc=nn.BCELoss()
log_training_settings(lr,num_epochs,batch_size,optim_gene,scheduler,loss_gene)
...
# 训练
train_GAN(train_iter, test_iter, generator, discriminator, loss_gene, loss_disc, optim_gene, optim_disc, num_epochs, scheduler,True)
...
# 测试
logging.info('Final test loss %e' % eval(test_iter, generator, nn.MSELoss(), 0))

# 应用完整图片并写入
IMG_NAME = r'img_0'
IMG_OUT = OUTPUTS_FOLDER+r'/'+IMG_NAME+r'_' + \
    computer_name+r'_'+format_time+r'.png'
output = apply_net(LR_FOLDER+r'/'+IMG_NAME+r'.png', IMG_OUT, generator, ratio)
